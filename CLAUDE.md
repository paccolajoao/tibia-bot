...
- `UsarMana` (Magic-Level training) casts `usar_mana.tecla` while mana ≥ `mana_alto`, with hysteresis down to `mana_alvo`. **Gotcha:** a full bar rarely reads exactly `100%` (anti-aliased edges + overlaid digits → ~96–99%), so a configured `mana_alto >= 100` is treated as `LIMIAR_CHEIO_INATINGIVEL` (~97%); the default is `95`. Its cooldown is keyed by `CHAVE_COOLDOWN` (`"usar_mana"`), not by the hotkey.
