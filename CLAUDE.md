...
Behaviors are evaluated by **priority** (highest first); the **first non-null decision wins the tick** (one action per tick → human-like timing, no hotkey storms). Per-hotkey cooldowns gate actions.
...
- [src/bot/visao/barra_recursos.py](src/bot/visao/barra_recursos.py) — reads bar fill % by sampling pixels and classifying filled-vs-empty in **HSV (brightness + saturation)**, so it survives HP changing color. Occluded/dirty reads (tooltip covering the bar) yield **low confidence** and are ignored downstream (`visao.confianca_minima`), so the bot never heals on a bad read.
...
- [portal/](portal/) — the web portal (Vite + React + TS + Tailwind v4 + shadcn-style Radix primitives). Pages: Dashboard (live WS), Configurações (forms for every section), Perfis (profiles + import/export), Calibração (draw HP/Mana/battle-list rectangles on a captured frame). **TS types in `portal/src/lib/types.ts` mirror the pydantic `Config` — update both sides when adding a field.**
...
- `targeting` and `auto-loot` only activate when the **battle list region is calibrated** (`calibrar.py`, ESC skips it).
